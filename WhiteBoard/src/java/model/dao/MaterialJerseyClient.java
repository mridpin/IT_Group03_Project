 ...
 * @author Portatil
 */
public class MaterialJerseyClient {

    private WebTarget webTarget;
    private Client client;
    private static final String BASE_URI = "http://localhost:8080/Whiteboard_REST/webresources";

    public MaterialJerseyClient() {
        client = javax.ws.rs.client.ClientBuilder.newClient();
        webTarget = client.target(BASE_URI).path("model.pojos.material");
    }

    public <T> T findFromAsignatura_XML(GenericType<T> responseType, String asignaturaId) throws ClientErrorException {
        WebTarget resource = webTarget;
        resource = resource.path(java.text.MessageFormat.format("material/{0}", new Object[]{asignaturaId}));
        return resource.request(javax.ws.rs.core.MediaType.APPLICATION_XML).get(responseType);
    }

    public <T> T findFromAsignatura_JSON(GenericType<T> responseType, String asignaturaId) throws ClientErrorException {
        WebTarget resource = webTarget;
        resource = resource.path(java.text.MessageFormat.format("material/{0}", new Object[]{asignaturaId}));
